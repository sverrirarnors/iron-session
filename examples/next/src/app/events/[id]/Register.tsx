...
export const canRegister = (
  ...
  user?.isLoggedIn && ((event?.onlyMembers && user?.info.isMember) || (event && event.onlyMembers))
...
const Checkbox = ({ label, name, value, onChange }: CheckBoxProps) => (
  <>
    ...
    />
    <label htmlFor={name} className="mx-auto sm:ml-1 sm:mr-2 sm:my-auto">
      {label}
    </label>
  </>
)
...
export default function Register({ event, user }: RegisterProps) {
  ...
  const { data: registrationData, refetch: refetchRegistration } = useQuery(GET_REGISTRATION, {
  ...
  const [deleteRegistration, { loading: deleteLoading }] = useMutation(DELTE_REGISTRATION, {
    variables: {
      eventId: event.id,
      token: user?.token,
    },
  ...
  const renderer = ({
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row">
            ...
            {isRegistered && event.registerOthers && !canRegister(user, event) && (
              <RegisterOther
                token={user.token}
                register={(anotherUser: any) =>
                  register({
                    variables: { eventId: event.id, token: user?.token, anotherUser },
                  })
                }
            ...
            {isRegistered && event.busAvailable && (
              <Checkbox
                label="Skrá í rútu"
                name="usesBus"
                onChange={() => handleMetaClick('usesBus')}
                value={usesBus}
              />
            )}
            {isRegistered && event.veganAvailable && (
              <Checkbox
                label="Vil vegan mat"
                name="isVegan"
                onChange={() => handleMetaClick('isVegan')}
                value={isVegan}
              />
            )}
    ...
    return (
      <p className="text-lg font-bold text-gray-50">
        ...
        {hours > 0 ? `${hours} klst.,` : ''} {minutes > 0 ? `${minutes} mín. og` : ''} {seconds}{' '}
        sek. í skráningu
      </p>
    )
  }
    ...
    <ApolloWrapper>
      {!canRegister(user, event) && (
        <p className="mb-3 text-red-300">
          Þú ert ekki virkur meðlimur og getur ekki skráð þig á þennan viðburð
        </p>
      )}
